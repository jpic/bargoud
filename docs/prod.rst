...
When adding a new file in `bargoud/static/`, you should
command django to update `bargoud/public/static/`
symlinks::
...
Create a file called `bargoud.ini`. It is important
that it is called `bargoud.ini` because it contains
`%n` which is a magic variable and will be replaced with `{{
project_name }}` by uWSGI. This would be a good start::

.. literalinclude:: examples/bargoud.ini
